...
from core.models import Pet, ReproductiveRecord, Admin
...
def check_record_exists(
    ...
    """Check if a ReproductiveRecord exists with the given ID (admin only)"""
...
def create_reproductive_record(
    payload: dict,
    db: Session = Depends(get_db),
    current_admin: Admin = Depends(auth.get_current_admin)
):
...
def update_reproductive_record(
    record_id: int,
    payload: dict,
    db: Session = Depends(get_db),
    current_admin: Admin = Depends(auth.get_current_admin)
...
def delete_reproductive_record(
    record_id: int,
    db: Session = Depends(get_db),
    current_admin: Admin = Depends(auth.get_current_admin)
):
