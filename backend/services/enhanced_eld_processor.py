...
ELD_MODEL = os.getenv("ELD_MODEL", os.getenv("AI_MODEL", os.getenv("GEMINI_MODEL", DEFAULT_MODEL)))  # Backward compatibility

# No fallback key - must be set in environment variables
if not ELD_API_KEY:
    logging.error("API_KEY not found in environment variables. Please set it in Railway or .env file.")
    ELD_API_KEY = None

if ELD_AVAILABLE and ELD_API_KEY:
    try:
        configure(api_key=ELD_API_KEY)
        actual_model = get_model_name(ELD_MODEL)
    ...
    if not ELD_API_KEY:
        logging.warning("ELD processing key not found in environment variables")
    if not ELD_AVAILABLE:
        logging.warning("Enhanced ELD dependencies not available")
    ELD_AVAILABLE = False

# Debug logging
logging.info(f"ELD_AVAILABLE: {ELD_AVAILABLE}")
logging.info(f"ELD_API_KEY present: {bool(ELD_API_KEY)}")
logging.info(f"ELD_MODEL: {get_model_name(ELD_MODEL) if ELD_AVAILABLE else ELD_MODEL}")
...
def enhanced_eld_assessment(image_bytes: bytes, additional_context: Optional[str] = None) -> Dict[str, Any]:
    """
    Enhanced ELD assessment using ELD model - let ELD do all the work
    """
    
    if not ELD_AVAILABLE:
        logging.warning("Enhanced ELD not available, using fallback")
        return {
            "success": True,
            "pain_level": "Level 1 (Mild Pain)",
            "pain_score": 5,
    ...
    models_to_try = [ELD_MODEL]
    
    # Add fallback models if primary model fails
    fallback_models = get_fallback_models(ELD_MODEL)
            ...
            if "quota" in error_str.lower() or "free_tier" in error_str.lower() or "limit" in error_str.lower():
                ...
                if model_name != models_to_try[-1]:  # Not the last model to try
                    ...
                    return {
                        ...
                        "error_message": "ELD service quota exceeded. Please try again later.",
                        "pain_level": "Level 1 (Mild Pain)",
                        "pain_score": 5,
                        "confidence": 0.2,
                        "analysis": "ELD assessment temporarily unavailable. Please try again later.",
                        "model_type": "ELD",
                        "raw_response": str(e)
                    }
            else:
    ...
    logging.error(f"Error in ELD assessment with all models: {last_error}")
    return {
        "success": False,
        "pain_level": "Level 1 (Mild Pain)",
        "pain_score": 5,
...
def is_enhanced_eld_available() -> bool:
    """Check if Enhanced ELD is available"""
    return ELD_AVAILABLE and bool(ELD_API_KEY)

def process_image_with_enhanced_eld(image_bytes: bytes) -> Dict[str, Any]:
    """Main function to process image with enhanced ELD"""
    return enhanced_eld_assessment(image_bytes)
